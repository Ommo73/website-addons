...
function quickview(){
    ...
    sAnimations.registry.websiteSaleCategory.include({
        ...
        _onOpenClick: function (ev) {
            if (document.documentElement.clientWidth < 991) {
                ...
                var parent = $fa.parent();
                var siblings = $fa.parent().parent().children()
                for (var i = 0; i < siblings.length; i++) {
                    if (siblings[i] !== parent[0]){
                        siblings[i].hidden = true;
                    }
                }
                $fa.parent().children().find(".fa-chevron-down").click()
        ...
        _onCloseClick: function (ev) {
            ...
            var siblings = $fa.parent().siblings(".nav-item ")
            for (var i = 0; i < siblings.length; i++) {
    ...
    sAnimations.registry.themeEvent = sAnimations.Class.extend({
        ...
        _onFilterClose: function(ev) {
            ...
            var category_list = document.getElementById('o_shop_collapse_category')
            var siblings = category_list.getElementsByTagName('*')
            for (var i = 0; i < siblings.length; i++) {
                    if (siblings[i].hidden == true){
                        siblings[i].hidden = false;
                    }
            }
        },
